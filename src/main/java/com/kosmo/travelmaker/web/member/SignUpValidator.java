...
public class SignUpValidator implements Validator{
	...
	public boolean supports(Class<?> command) {
		...
		return SignUpCommand.class.equals(command);
	}
	...
	public void validate(Object command, Errors errors) {
		...
		if (cmd.getAge().trim().equals("")) {
			errors.rejectValue("age","ageError");
		} 
		else {
			try {
				Integer.parseInt(cmd.getAge());
			} 
			catch (NumberFormatException e) {
				errors.rejectValue("age", "ageNotNumber");
			}
		}
		...
		else if(cmd.getPassword().equals(cmd.getPassword_check())) {
			errors.rejectValue("password", "password_checkError");
		}
		if (cmd.getId().equals("")) {
